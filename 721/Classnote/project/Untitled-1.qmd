...
ft_wbc <- flextable(wbc_table) |>
  align(j = 2:3, align = "center", part = "all") |>
  autofit() |>
  set_caption("Table A. Counts (%) of Encounters within Each WBC Category")
...
tbl1 <- tbl_summary(
  ...
  bold_labels() |>
  as_flex_table() |>
  set_caption("Table 1. Baseline Characteristics of Patients") |>  # ← 这行决定 Word 里的表题
  autofit()
...
tbl1 <- tbl_summary(
  ...
  bold_labels() |>
  as_flex_table() |>
  set_caption("Table 1. Baseline Characteristics of Patients") |>
  autofit()
...
# 可选：期刊风主题（JAMA 风格，去掉灰条、统一大小写等）
theme_gtsummary_journal(journal = "jama")
theme_gtsummary_compact()   # 更紧凑（可注释掉）
